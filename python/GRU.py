...
#d=pd.read_csv('midibeat.csv',header=None, sep="\t").as_matrix()
#data=list(d[:, 1])
...
words = []
outchar = []
for i in range(0, len(data) - seqLen,4):
...
earlystopper=EarlyStopping(monitor="val_loss", patience=1, mode='auto')
...
if rerun == True or not os.path.isfile('weights_testivedot2.hdf5'):
    model.fit(X, y, batch_size=32, nb_epoch=32
