...
def csv_to_pianoroll(filename, include_pauses=False):
    """
    ...
    """
    # check encoding width, do we include pauses to bin string.
    if include_pauses:
        roll_width = constants.BIN_STRING_LEN
    else:
        roll_width = constants.MAX_DRUMS
    # read input
    d = pd.read_csv(filename, header=None, sep="\t").values
    # return space
    pr = []
    ...
    for i in range(seqLen):
        pr.append(np.zeros(roll_width).astype('int'))
    # iterate csv
    for i in d[:, 1]:
        # rows of zeros for pause frames
        if i < 0:
            if include_pauses:
                num = np.array(utils.dec_to_binary(i, roll_width, ret_type='int'))[::-1]
                pr.append(num)
            else:
                for i in range(-1 * i):
...
def merged_from_pianoroll(pr, simple=True):
    ...
    pause_length = 0
    for i in pr:
        note = utils.enc_to_int(i, is_long=True)
        if note == 0:
            if simple:
                merged.append(0)
            else:
...
def get_multi_sequences(filename, seqLen=seqLen, sampleMul=1., include_pauses=False):
    pr = csv_to_pianoroll(filename, include_pauses=include_pauses)
    words = []
    outchar = []
    ...
    try:
        ...
        X, y = resample(np.array(X), np.array(y), n_samples=samples, replace=True, random_state=1337)
...
def initModel(seqLen=seqLen, kitPath=None, destroy_old=False, model_type='single_mgu',
              multi_drums=constants.MAX_DRUMS):
    ...
    try:
        if destroy_old:
            ...
            model = load_model('{}model_{}.hdf5'.format(filePath, model_type), custom_objects={'MGU': MGU, 'fixed_focal_loss':focal_loss()})
            optr = adam(lr=0.001)
        ...
        if model_type == 'conv_mgu':
            ...
            def get_drum_layer(seqLen):
                ...
                mgu1 = (MGU(layer_size, activation='tanh',
                            return_sequences=False, dropout=0.7, recurrent_dropout=0.7, implementation=1))(rs1)
                ...
                # mgu1 = Flatten()(mgu1)
                # mgu1=Dense(layer_size,kernel_initializer='he_normal', activation='relu')(mgu1)
                return [in1, mgu1]

            def get_out_layer(in_layer):
                # noise=GaussianNoise(0.1)(in_layer)
                # dense1=MGU(layer_size, return_sequences=False)(in_layer)
            ...
            def get_drum_layer(seqLen):
                in1 = Input(shape=(seqLen,))
                rs1 = Reshape((1, seqLen,))(in1)
                mgu1 = MGU(layer_size, activation='tanh', input_shape=(seqLen, 1),
                           return_sequences=False, dropout=0.7, recurrent_dropout=0.7, implementation=1)(rs1)
            ...
            def get_out_layer(in_layer):
                dense1 = Dense(dense_layer_size, kernel_initializer='he_normal', activation='relu')(in_layer)
                dense2 = Dense(1, kernel_initializer='he_normal', activation='sigmoid')(dense1)
                return [in_layer, dense2]
            ...
            merged = Concatenate()(list(in_layers[:, 1]))
            merged = Dense(layer_size, activation='relu')(merged)
            # out_layers = []
            # for i in range(multi_drums):
            #    out_layers.append(get_out_layer(merged))
            # out_layers = np.array(out_layers)
            ...
            def get_layer(seqLen, n_kernels, n_win):
                ...
                conv1 = TimeDistributed(
                    Conv1D(n_kernels, n_win, activation='elu', kernel_regularizer=regularizers.l2(regVal),
                           activity_regularizer=regularizers.l1(regVal2)), input_shape=(1,) + (seqLen, numDiffHits))(
                    reshape1)
        ...
        # using the attention class
            ...
            def get_layer(seqLen):
                ...
                mgu1 = (
                    MGU(layerSize, activation='tanh', return_sequences=ret_seq, dropout=drop, recurrent_dropout=drop,
            ...
            model.add(Embedding(numDiffHits + 1, numDiffHits, input_length=seqLen))
            model.add(GRU(layerSize, activation='tanh', input_shape=(seqLen, numDiffHits),
                          # kernel_initializer='lecun_normal',
...
def train(filename=None, seqLen=seqLen, sampleMul=1., forceGen=False, bigFile=None, updateModel=False,
    ...
    class LossHistory(Callback):
        ...
        def on_batch_end(self, batch, logs={}):
            ...
            # If loss vs. val_loss is small slow down learning, dynamic scheduling!

            self.losses.append(logs.get('loss'))
            self.accs.append(logs.get('acc'))

        def on_epoch_end(self, epoch, logs={}):
            # Test lr scheduling and reinit of mgu cells
            # K.set_value(model.optimizer.lr,0.001)
            # print(' lr:', K.get_value(model.optimizer.lr))
            # prune_layers(model)
            print('loss:',logs.get('loss'),' val_loss:',logs.get('val_loss'))
            # if (logs.get('loss') - logs.get('val_loss') < .1 and K.get_value(model.optimizer.lr)>=0.001):
    ...
    modelsaver = ModelCheckpoint(filepath="{}weights_testivedot_{}.hdf5".format(CurrentKitPath, model_type), verbose=1,
                                 save_best_only=True)
    temporarysaver = ModelCheckpoint(filepath="{}temp.hdf5".format(CurrentKitPath), verbose=0, save_best_only=False)
    genMdelSaver = ModelCheckpoint(filepath="{}weights_testivedot_ext.hdf5".format(CurrentKitPath), verbose=1,
                                   save_best_only=True)
    ...
    if filename is not None and updateModel is not 'generator':
        try:
            if model_type == 'multi2multi':
                X_train, y_train = get_multi_sequences(filename=filename, seqLen=seqLen, sampleMul=sampleMul,
    ...
    with graph.as_default():
        ...
        #    class_weights = []
        #    tot_hits = sum(sum(y_train))
        #    gtotal=len(y_train[0])
        #    for i in range(y_train.shape[0]):
        #        inst_amount=sum(y_train[i])
        #
        #        hit_weight=len(y_train[i])/(inst_amount)
        #        class_weights.append({0:(gtotal-tot_hits)/gtotal, 1:hit_weight/gtotal})
        #    print(class_weights)
        # else:
        #
        ...
        if updateModel == 'generator':
            ...
            model.fit_generator(generator=tr_gen, epochs=20, steps_per_epoch=10, max_queue_size=10,
                                callbacks=[genMdelSaver, earlystopper],
        ...
        if updateModel == True:
            # model.load_weights("{}weights_testivedot_{}.hdf5".format(CurrentKitPath,model_type))
            #
            model.fit(X_train_comp, y_train, batch_size=int(max([32])),
                      epochs=initial_epoch + 5,
                      callbacks=[modelsaver, earlystopper, history],  # ,learninratescheduler],#earlystopper,
                      validation_split=0.33,
                      verbose=0, initial_epoch=initial_epoch, class_weight=class_weights, shuffle=False)
            lastLoss = np.mean(history.losses[-10:])
            pickle.dump(initial_epoch + 5, open(CurrentKitPath + '/initial_epoch.k', 'wb'))
            model.load_weights("{}weights_testivedot_{}.hdf5".format(CurrentKitPath, model_type))
            model.save('{}model_{}.hdf5'.format(CurrentKitPath, model_type))
        ...
        if updateModel == 'm2m':
            ...
            # model.load_weights("{}weights_testivedot_{}.hdf5".format(CurrentKitPath,model_type))
            #
            model.fit([X_train_comp], y_comp, batch_size=int(max([y_train.shape[0] / 50, 32])),
                      epochs=initial_epoch + 2000,
            ...
            for i in range(20):
                model.fit(X_train_comp, y_train, batch_size=int(max([y_train.shape[0] / 50, 32])),
                          epochs=1,
                          callbacks=[modelsaver, earlystopper, history,
                                     LearningRateScheduler(lambda _: 5e-3 * (0.6 ** i))],
                          # learninratescheduler],earlystopper,
                          validation_split=0.33,
...
def generatePart(data, partLength=500, seqLen=seqLen, temp=None, include_seed=False, model_type='parallel_mgu'):
    ...
    def sample_multi(a, flam_inhibitor, temperature, pause_threshold):
        ...
        # Since there is no pause in the array we must artificially introduce them
    ...
    # Flam inhibitor prevents hits that are essentially the same hit but happen after a one frame shorter pause
    ...
    while i < partLength:
        ...
        if model_type == 'multi2multi':
            ...
            # variance or not?
            if temp == 0.:
                hit_ind = np.argwhere(next_notes >= 0.3)
                next_notes = np.zeros_like(next_notes)
                next_notes[hit_ind] = 1
            else:
                # sample hit
            ...
            # encode notes to single int, change to right endianness
            next_char = utils.enc_to_int(next_notes, is_long=False)
            ...
            if next_char < 0:
                ...
                next_notes = np.array(utils.dec_to_binary(next_char, str_len=constants.MAX_DRUMS))
            # next_notes=np.array(utils.dec_to_binary(next_char,str_len=constants.BIN_STRING_LEN))
            data = np.concatenate((data[:, :, 1:], next_notes.reshape(constants.MAX_DRUMS, 1, 1)), axis=2)
            # data = np.concatenate((data[:, :, 1:], next_notes.reshape(constants.BIN_STRING_LEN, 1, 1)), axis=2)
        else:
            next_index = sample(pred[0], fuzz)
            next_char = Ichar.get(next_index, 0)
            generated.append(next_char)
...
def debug():
    ...
    if True:
        ...
        model_type = ['TDC_parallel_mgu', 'time_dist_conv_mgu', 'parallel_mgu', 'stacked_mgu', 'conv_mgu', 'single_mgu',
                      'single_gru', 'single_lstm', 'tcn', 'ATT_TDC_P_mgu', 'time_dist_conv_mgu_att', 'multi2one',
                      'multi2multi']
        ...
        if False:
            model = initModel(seqLen=seqLen, destroy_old=True, model_type=model_type[0],
                              multi_drums=constants.MAX_DRUMS)
            seed, history = train('testbeat3.csv', seqLen=seqLen, sampleMul=1, model_type=model_type[0],
                                  updateModel=True, return_history=True)

            file = generatePart(seed, partLength=500, seqLen=seqLen, temp=temp, include_seed=False,
        ...
        for j in model_type:
            log = []
            if j == 'multi2one':
                model = initModel(seqLen=seqLen, destroy_old=True, model_type=j, multi_drums=constants.MAX_DRUMS)
            else:
                model = initModel(seqLen=seqLen, destroy_old=True, model_type=j, multi_drums=constants.MAX_DRUMS)
            takes = ['./Kits/MDC_Stack/takes/{}'.format(f) for f in os.listdir('./Kits/MDC_Stack/takes/') if
                     not f.startswith('.')]
            takes.sort()
            takes2 = ['./Kits/timedist/takes/{}'.format(f) for f in os.listdir('./Kits/timedist/takes/') if
                      not f.startswith('.')]
            takes2.sort()
            data = []
            ...
            for i in takes2:
                ...
                t1 = time()
                seed, history = train(i, seqLen=seqLen, sampleMul=1, model_type=j, updateModel=True,
                                      return_history=True)
                # seed=train(i,seqLen=seqLen,sampleMul=1,model_type=j,updateModel=True, return_history=True, forceGen=True)
    ...
    if False:
        ...
        for i in takes:
            ...
            xss.append(X)
            yss.append(y)
        generatePart(train(filename=None, seqLen=seqLen, sampleMul=1, forceGen=False,
                           bigFile=[np.concatenate((xss)), np.concatenate((yss))], updateModel=True), temp=1.)
        # for i in takes:
