...
class SoundCheckScreen(Screen):
    ...
    def stopSoundCheck(self):
        print('trying to stop soundchek:', drumsynth._ImRecording)
        ...
        def callback():
            try:
                ...
                if self.deltaTempo < 0.83:
                    ...
                    countTempo = .5
                self.lastMessage = drumsynth.createWav(fullPath, outFile, addCountInAndCountOut=addCountInAndCountOut,
                                                       deltaTempo=self.deltaTempo, countTempo=countTempo)
    ...
    def doPlayerTurn(self):
        ...
        def callback():
            try:
                ...
                # TODO:make user adjustable length
                self.lastPlayerPart, self.deltaTempo = game.playLive(fullPath, thresholdAdj=self.threshold,
                                                                     part_length=30, saveAll=True,
                                                                     quantize=self.quantize)
                if self.lastPlayerPart == False:
                    return
                self.createLast(self.lastPlayerPart, outFile='./player_performance.wav',
                                addCountInAndCountOut=(not self.step))
                if self.step:
                    self.playBackMessage = 'Play Back Last Performance'
                    self.pBtnMessage = 'Play'
                elif self.pBtnMessage == 'Stop':
